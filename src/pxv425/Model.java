...
public abstract class Model extends Observable {
	 ...
	 * @return true, if the time and the day are ok
	 * return false, if the time and the day are not ok
	 * 
	 * @author Panagiotis Vakalis
	 * @version 14-07-2015
	 */
	private boolean isValidDayAndTime(){
		...
		if(today == Calendar.SATURDAY || today == Calendar.SUNDAY){
			/*
			 * If the current day is either Saturday or Sunday
			 * then the stock market is closed.
			...
			if(time.format(calendar.getTime()).compareTo("16:29:59") == 1 || time.format(calendar.getTime()).compareTo("09:59:59") == -1){
				/*
				 * If the time is after 16:29:59 or before 09:59:59
				 * then the stock market is closed and the method
				 * returns false 
				 */
				return false;
			}
				...
				/*
				 * If the time is between the already mentioned times
				 * then the marker is open and the method returns
				 * true
				 */
				return true;
			}
